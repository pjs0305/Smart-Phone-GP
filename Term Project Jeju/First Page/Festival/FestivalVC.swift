...
class FestivalVC: UIViewController, XMLParserDelegate, UITableViewDataSource {
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?)
    {
        if (elementName as NSString).isEqual(to: "list")
        {
            ...
            FestivalVC.posts.add(elements)
        }
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
        return FestivalVC.posts.count
    }
    ...
    func tableView(_ tableView:UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        let title = (FestivalVC.posts.object(at: indexPath.row) as AnyObject).value(forKey: "title") as! NSString as String
        let location = (FestivalVC.posts.object(at: indexPath.row) as AnyObject).value(forKey: "location") as! NSString as String
