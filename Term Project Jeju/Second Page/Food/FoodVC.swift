...
class FoodVC: UIViewController, XMLParserDelegate, UITableViewDataSource {
    ...
    var url : String! = "http://data.jeju.go.kr/rest/besteating/getEatingList?serviceKey=7Z3e6MM%2BZVra4DYqS7dDT%2Bsfh%2Fw2JlIBVc4uE9Xc%2FEKVgineKHp9fvznQMmblhdNhsBaCa2S31NGHVGY2j9gLg%3D%3D&pageSize=100"
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?)
    {
        if (elementName as NSString).isEqual(to: "list")
        {
            ...
            FoodVC.posts.add(elements)
        }
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
        return FoodVC.posts.count
    }
    ...
    func tableView(_ tableView:UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        let adres = (FoodVC.posts.object(at: indexPath.row) as AnyObject).value(forKey: "adres") as! NSString as String
        let dataTitle = (FoodVC.posts.object(at: indexPath.row) as AnyObject).value(forKey: "dataTitle") as! NSString as String
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "segueToMapView"
        {
            ...
            {
                let la = (FoodVC.posts.object(at: 0) as AnyObject).value(forKey: "la") as! NSString as String
                let lo = (FoodVC.posts.object(at: 0) as AnyObject).value(forKey: "lo") as! NSString as String
